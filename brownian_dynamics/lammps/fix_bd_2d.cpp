...
FixBD2D::FixBD2D(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixBD2D::initial_integrate(int vflag)
{
  double dtfm;
  double randf;
  double omega0;
  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        omega0 = random->gaussian();
	omega1 = random->gaussian();
        x[i][0] += dtv * dtfm * (f[i][0]+randf*omega0);
	x[i][1] += dtv * dtfm * (f[i][1]+randf*omega1);
        v[i][0] = dtfm * (f[i][0]+randf*omega0);
	v[i][1] = dtfm * (f[i][1]+randf*omega1);
      }
