  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        r = sqrt(rsq);
        rexp = log(r + rhoinv[itype][jtype]) - log(r+buck2[itype][jtype]);
        forcebuck = buck1[itype][jtype]*r*(1.0/(r+buck2[itype][jtype]) - 1.0/(r + rhoinv[itype][jtype]));
...
double PairBuck::init_one(int i, int j)
{
  ...
  if (offset_flag && (cut[i][j] > 0.0)) {
    double rexp = log(cut[i][j] + rho[i][j]) - log(cut[i][j] + c[i][j]);
    offset[i][j] = a[i][j]*rexp;
  } else offset[i][j] = 0.0;
                        ...
                        double &fforce)
{
  ...
  r = sqrt(rsq);
  rexp = log(r + rhoinv[itype][jtype]) - log(r + buck2[itype][jtype]);
  forcebuck = buck1[itype][jtype]*(r/(r + buck2[itype][jtype]) - r/(r + rhoinv[itype][jtype]));
  fforce = factor_lj*forcebuck*r2inv;

  phibuck = a[itype][jtype]*rexp -
    offset[itype][jtype];
